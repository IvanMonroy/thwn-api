module Api
  class ProductsController < ApplicationController
    ...
    def get_by_desc
      @products = Product.where("UPPER(name) LIKE ? OR UPPER(description) LIKE ? OR UPPER(mark) LIKE ? OR UPPER(category) LIKE ?", "#{params[:filter].upcase}%", "#{params[:filter].upcase}%", "#{params[:filter].upcase}%", "#{params[:filter].upcase}%")
      render_default_format(@products,true,200)
    rescue Exception => e
